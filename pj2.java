public class Gameboard{
	...
	public boolean performMove(int color, Move m){
		if(isValidMove(m.x1, m.y1, color)){
			...
			for(int j=0; j<8; j++){
				for(int i=0; i<8; i++){
					if(cellContent(i, j) !=null && cellContent(i, j).getColor()==color){
						...
						for(int n=0; n<8; n++){
							for(int m=0; m<8; m++){
								if(isValidMove(m, n, color) && (m != i || n!= j)){
									Move m = new Move(i, j, m, n);
									mylist.insertBack(m);
									Chip chip = new Chip(color, i, j);
									board[i][j]==chip;
								}
							}
						}
  ...
  BestMove bestReply;
  DList listMove=new genarateMoves(color);
  DListNode node;
  
  
  if(((depth==0)||(potentialNetworkCount==1))&&(color==WHITE)){
    mybest.score=countMove*evaluate();
    return mybest;
    }
  else if(((depth==0)||(potentialNetworkCount==1))&&(color==BLACK)) { 
  			mybest.score=-countMove*evaluate();
    	    return mybest;
        }
